...
class Sci_UiCtl(sci_tool.Ui_MainWindow):
    def __init__(self, MainWindow):
        ...
        self.sciopenButton.clicked.connect(
            lambda: self.SciOpenButton_Click())  # self.sciopenButton, QtCore.SIGNAL('clicked()'), self.SciOpenButton_Click)#connect button click func
        self.clrcontentbutton.clicked.connect(
            lambda: self.ClrButtonProcess())  # self.clrcontentbutton, QtCore.SIGNAL('clicked()'), self.ClrButtonProcess)
        self.mainsend_Button.clicked.connect(
            lambda: self.MainSendButtonProcess())  # self.mainsend_Button, QtCore.SIGNAL('clicked()'), self.MainSendButtonProcess)
        self.sendclr_Button.clicked.connect(
            lambda: self.ClrSendButtonProcess())  # self.sendclr_Button, QtCore.SIGNAL('clicked()'), self.ClrSendButtonProcess)
        self.clrcntbutton.clicked.connect(
            lambda: self.ClrCntButtonProcess())  # self.clrcntbutton, QtCore.SIGNAL('clicked()'), self.ClrCntButtonProcess )
        self.cmd1sned_Button.clicked.connect(
            lambda: self.Cmd1SendButtonProcess())  # self.cmd1sned_Button, QtCore.SIGNAL('clicked()'), self.Cmd1SendButtonProcess)
        self.cmd2sned_Button.clicked.connect(
            lambda: self.Cmd2SendButtonProcess())  # self.cmd2sned_Button, QtCore.SIGNAL('clicked()'), self.Cmd2SendButtonProcess)
        self.cmd3sned_Button.clicked.connect(
            lambda: self.Cmd3SendButtonProcess())  # self.cmd3sned_Button, QtCore.SIGNAL('clicked()'), self.Cmd3SendButtonProcess)
        self.cmd4sned_Button.clicked.connect(
            lambda: self.Cmd4SendButtonProcess())  # self.cmd4sned_Button, QtCore.SIGNAL('clicked()'), self.Cmd4SendButtonProcess)
        self.cmd5sned_Button.clicked.connect(
            lambda: self.Cmd5SendButtonProcess())  # self.cmd5sned_Button, QtCore.SIGNAL('clicked()'), self.Cmd5SendButtonProcess)
        self.savecontentbutton.clicked.connect(
            lambda: self.SaveRecButtonProcess())  # self.savecontentbutton, QtCore.SIGNAL('clicked()'), self.SaveRecButtonProcess)
        self.x1save_button.clicked.connect(
            lambda: self.X1SaveButtonProcess())  # self.x1save_button, QtCore.SIGNAL('clicked()'), self.X1SaveButtonProcess)
        self.x1clr_button.clicked.connect(
            lambda: self.X1ClrButtonProcess())  # self.x1clr_button,  QtCore.SIGNAL('clicked()'), self.X1ClrButtonProcess)
        self.x2save_button.clicked.connect(
            lambda: self.X2SaveButtonProcess())  # self.x2save_button, QtCore.SIGNAL('clicked()'), self.X2SaveButtonProcess)
        self.x2clr_button.clicked.connect(
            lambda: self.X2ClrButtonProcess())  # self.x2clr_button,  QtCore.SIGNAL('clicked()'), self.X2ClrButtonProcess)
        self.x3save_button.clicked.connect(
            lambda: self.X3SaveButtonProcess())  # self.x3save_button, QtCore.SIGNAL('clicked()'), self.X3SaveButtonProcess)
        self.x3clr_button.clicked.connect(
            lambda: self.X3ClrButtonProcess())  # self.x3clr_button,  QtCore.SIGNAL('clicked()'), self.X3ClrButtonProcess)
    ...
    def SaveRecButtonProcess(self):
        filename = QtGui.QFileDialog.getSaveFileName(self.savecontentbutton, 'Save File', '.',
    ...
    def X1SaveButtonProcess(self):
        filename = QtGui.QFileDialog.getSaveFileName(self.x1save_button, 'Save File', '.',
                                                     "Text file(*.txt);;All file(*.*)")
        fname = open(filename, 'w')
    ...
    def X2SaveButtonProcess(self):
        filename = QtGui.QFileDialog.getSaveFileName(self.x2save_button, 'Save File', '.',
                                                     "Text file(*.txt);;All file(*.*)")
        fname = open(filename, 'w')
    ...
    def X3SaveButtonProcess(self):
        filename = QtGui.QFileDialog.getSaveFileName(self.x3save_button, 'Save File', '.',
                                                     "Text file(*.txt);;All file(*.*)")
        fname = open(filename, 'w')
