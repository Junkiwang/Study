...
class FormulaDemo(QWidget):
    def __init__(self, parent=None):
        ...
        self.btn3 = QPushButton('清除x')
        self.btn4 = QPushButton('清除y')
        self.btn3.clicked.connect(self._clearpointx)
        self.btn4.clicked.connect(self._clearpointy)
        layout.addWidget(self.btn3, 8, 1, 1, 1)
        layout.addWidget(self.btn4, 8, 3, 1, 1)
    ...
    def _calcformula(self):
        try:
            ...
            if x1 == x2:
                ...
                reply = QMessageBox.warning(self, '错误提示', 'x轴线不计算')
            elif y1 == y2:
                self.label6.setText('y = %0.2f' % y1)
                reply = QMessageBox.warning(self, '错误提示', 'y轴线不计算')
            else:
                global k, b
                k = (y1 - y2) / (x1 - x2)
                b = y1 - (k * x1)
    ...
    def _calcpoint(self):
        try:
            ...
            elif self.input5.text() == '':
                y = float(self.input6.text())
                self.input5.setText('%0.2f' % (lambda y: (y - b) / k)(y))
            else:
                reply = QMessageBox.warning(self, '错误提示', '请先清除任一坐标')
        except:
            reply = QMessageBox.warning(self, '错误提示', '无效输入')

    def _clearpointx(self):
        self.input5.setText('')

    def _clearpointy(self):
        self.input6.setText('')
